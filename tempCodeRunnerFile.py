...
def editar_categoria(id_categoria):
    ...
    if request.method == 'POST':
        # Manejar el envío del formulario para editar el producto aquízzzzz
        # Recuperar los datos del formulario, actualizar la base de datos, etc.
        flash('Producto editado correctamente', 'success')
        return redirect(url_for('gestionar_categorias'))
    else:
        # Obtener los datos de la categoría existente de la base de datos
        cur = mysql.connection.cursor()
        cur.execute("SELECT * FROM categorias WHERE id_categoria = %s", (id_categoria,))
        categoria = cur.fetchone()  # Usar fetchone() para obtener una sola fila
        cur.close()
...
# Ruta Flask para editar una categoría
@app.route('/descuentos/<int:id_descuento>/editar', methods=['GET', 'POST'])
def editar_descuento(id_descuento):
    if request.method == 'POST':
        ...
        try:
            ...
            """, (codigo, cantidad, fecha_inicio, fecha_fin, porcentaje, descripcion, id_descuento))
            mysql.connection.commit()
        ...
        cur = mysql.connection.cursor()
        cur.execute("SELECT * FROM descuentos WHERE id_descuento = %s", (id_descuento,))
